...
const brandCards = brands.map(([brand, items]) => {
  const productRows = list => list.map(p => `<a class="product-link" href="/products/${encodeURIComponent(p.id)}">${productMedia(p,"product-media product-thumb")}<span>${esc(p.name)}<br><small>参考価格 ¥${yen(p.price)}・Moilum編集部評価 ${esc(p.rating)}</small></span></a>`).join("");
