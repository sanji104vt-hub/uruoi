...
function isRakutenCatalogProduct(product){
  return product.sourceType === "rakuten_product_api";
}
...
function rakutenDestination(product){
  try {
    const purchase = new URL(product.purchase || "");
    if (["item.rakuten.co.jp","product.rakuten.co.jp"].includes(purchase.hostname)) return purchase.href;
    if (purchase.hostname === "hb.afl.rakuten.co.jp") return purchase.searchParams.get("pc") || purchase.href;
  } catch {}
  return `https://search.rakuten.co.jp/search/mall/${encodeURIComponent(product.brand + " " + product.name)}/`;
}

function moshimoRakutenLink(product){
  const query = new URLSearchParams({
    a_id:MOSHIMO_RAKUTEN.aId,
    p_id:MOSHIMO_RAKUTEN.pId,
    pc_id:MOSHIMO_RAKUTEN.pcId,
    pl_id:MOSHIMO_RAKUTEN.plId,
    url:rakutenDestination(product)
  });
...
function productMedia(product, className = "product-media"){
  const fallback = `<span class="product-image-fallback"${product.image ? " hidden" : ""} aria-hidden="true"><span>${esc(product.icon || "💧")}</span><small>画像準備中</small></span>`;
  const image = product.image
    ? `<img src="${esc(product.image)}" alt="${esc(product.name)}の商品画像" loading="lazy" decoding="async" referrerpolicy="no-referrer-when-downgrade" onerror="this.hidden=true;this.nextElementSibling.hidden=false">`
    : "";
  return `<span class="${esc(className)}">${image}${fallback}</span>`;
}
...
.directory-controls{position:sticky;top:70px;z-index:5;padding:16px;margin:24px 0;background:rgba(251,249,246,.96);border:1px solid var(--line);border-radius:16px;box-shadow:var(--shadow)}.directory-controls .search{margin:0 0 12px}.filter-row{display:flex;gap:8px;flex-wrap:wrap}.filter-chip{min-height:44px;padding:8px 13px;border:1px solid var(--deep);border-radius:999px;background:#fff;color:var(--ink);font:500 13px var(--sans);cursor:pointer}.filter-chip[aria-pressed="true"]{background:var(--water);border-color:#668891}.price-filter{min-height:44px;padding:8px 12px;border:1px solid var(--deep);border-radius:12px;background:#fff;color:var(--ink);font:inherit}.product-category{scroll-margin-top:160px}.product-category h2{display:flex;align-items:baseline;gap:10px;border-bottom:1px solid var(--line);padding-bottom:8px}.product-category h2 span{font:500 15px "Cormorant",serif;color:#65777c}.product-directory-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(245px,1fr));gap:14px}.product-directory-card{display:flex;flex-direction:column;min-height:100%;background:#fff;border:1px solid var(--line);border-radius:16px;padding:17px;box-shadow:var(--shadow)}.product-directory-card h3{font-size:17px;margin:7px 0}.product-directory-card .description{font-size:13px;color:#52666b;flex:1}.product-directory-card .product-cta,.product-directory-card .product-buy{display:inline-flex;align-items:center;justify-content:center;min-height:44px;margin-top:12px;padding:8px 12px;border-radius:12px;font-weight:700;text-decoration:none}.product-directory-card .product-cta{background:var(--water)}.product-directory-card .product-buy{background:#bf0000;color:#fff}.status-pill{background:var(--iris-1)}.guide-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px}.guide-list a{display:flex;align-items:center;min-height:44px;padding:10px 12px;background:#fff;border:1px solid var(--line);border-radius:12px;text-decoration:none}.brand-more{margin-top:10px}.brand-more summary{min-height:44px;display:flex;align-items:center;cursor:pointer;color:#385d68;font-weight:700}.brand-more .product-link{margin-left:8px}@media(max-width:768px){.product-directory-grid{grid-template-columns:1fr}.directory-controls{position:static}}
.product-media{position:relative;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#fff;border:1px solid var(--line);border-radius:14px}.product-media img{display:block;width:100%;height:100%;object-fit:contain}.product-image-fallback{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;width:100%;height:100%;background:linear-gradient(145deg,var(--base),var(--water));color:#52666b}.product-image-fallback[hidden]{display:none}.product-image-fallback>span{font-size:34px}.product-image-fallback small{font-size:10px}.directory-image-link{display:block;margin:-3px -3px 13px;text-decoration:none}.directory-product-media{width:100%;height:190px}.product-link{min-height:64px}.product-thumb{width:48px;height:48px;flex:0 0 48px;border-radius:10px}.product-thumb .product-image-fallback>span{font-size:21px}.product-thumb .product-image-fallback small{display:none}.rank-row{grid-template-columns:42px 64px minmax(0,1fr) auto}.rank-row .rank-image{width:64px;height:64px}@media(max-width:768px){.product-directory-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.product-directory-card{padding:11px}.directory-product-media{height:150px}.product-directory-card h3{font-size:14px}.product-directory-card .description{display:none}.rank-row{grid-template-columns:36px 52px minmax(0,1fr)}.rank-row .rank-image{width:52px;height:52px}.rank-row .price{grid-column:3}}@media(max-width:430px){.product-directory-grid{grid-template-columns:1fr}.directory-product-media{height:190px}.product-directory-card .description{display:block}}
`;
...
const directoryCard = product => {
  ...
  const status = product.status === "previous_generation"
    ...
    : isRakutenCatalogProduct(product) ? `<span class="pill status-pill">楽天在庫確認済み</span>` : "";
  return `<article class="product-directory-card" data-product-id="${esc(product.id)}" data-category="${esc(product.category)}" data-audience="${esc(product.audience || "unisex")}" data-price="${Number(product.price || 0)}" data-search="${esc(searchText)}"><a class="directory-image-link" href="/products/${encodeURIComponent(product.id)}" aria-label="${esc(product.name)}の商品詳細を見る">${productMedia(product,"product-media directory-product-media")}</a><div><span class="pill">${esc(product.category)}</span>${status}</div><h3>${esc(product.name)}</h3><p class="meta">${esc(product.brand)}・${esc(ratingText(product))}</p><p class="price">参考価格 ¥${yen(product.price)}</p><p class="description">${esc(product.desc)}</p><a class="product-cta" href="/products/${encodeURIComponent(product.id)}" aria-label="${esc(product.name)}の詳細を見る">商品詳細を見る</a>${isRakutenCatalogProduct(product)?`<a class="product-buy" href="${esc(moshimoRakutenLink(product))}" rel="nofollow sponsored noopener" referrerpolicy="no-referrer-when-downgrade" target="_blank" aria-label="${esc(product.name)}を楽天市場で見る（PR）">楽天で見る（PR）</a>`:""}</article>`;
};
...
const brandCards = brands.map(([brand, items]) => {
  const productRows = list => list.map(p => `<a class="product-link" href="/products/${encodeURIComponent(p.id)}">${productMedia(p,"product-media product-thumb")}<span>${esc(p.name)}<br><small>参考価格 ¥${yen(p.price)}・${esc(ratingText(p))}</small></span></a>`).join("");
...
const rankingSections = rankingGroups.map(([id,label,match]) => {
  ...
  return `<section id="${id}"><h2>${label}比較 TOP10</h2><p class="meta">掲載条件で抽出後、Moilum編集部評価順。同点は参考価格の低い順です。</p><div class="ranking-list">${ranked.map(p=>`<div class="rank-row">${productMedia(p,"product-media rank-image")}<div><a href="/products/${p.id}">${esc(p.name)}</a><div class="meta">${esc(p.brand)}・${esc(p.category)}・Moilum編集部評価 ${esc(p.rating)}</div></div><div class="price">¥${yen(p.price)}</div></div>`).join("")}</div></section>`;
}).join("");
page({
...
const aboutCounts = {
  ...
  "source-recorded": directoryProducts.filter(product => Number(product.sourceAudit?.sourceCount || 0) > 0).length,
  "source-missing": directoryProducts.filter(product => Number(product.sourceAudit?.sourceCount || 0) === 0).length,
  "source-level-a": directoryProducts.filter(product => product.sourceAudit?.qualityLevel === "A").length,
  "source-level-b": directoryProducts.filter(product => product.sourceAudit?.qualityLevel === "B").length,
  "source-level-c": directoryProducts.filter(product => product.sourceAudit?.qualityLevel === "C").length,
  "source-level-d": directoryProducts.filter(product => product.sourceAudit?.qualityLevel === "D").length
};
