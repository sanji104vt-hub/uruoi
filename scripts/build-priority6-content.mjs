...
function homeHtml(){
  const featured = editorProducts.map(product => `<article class="p6-featured-card"><span>${esc(product.category)}・編集部使用済み</span><h3><a href="/products/${product.id}">${esc(product.name)}</a></h3><p>${esc(product.brand)} ／ 参考価格 ¥${yen(product.price)}</p></article>`).join("");
